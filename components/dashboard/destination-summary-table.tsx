...
function buildRows(filter: string | undefined, limit: number): Row[] {
  ...
  return MOCK_DESTINATIONS.filter((d) => !filter || d.tfn === filter)
    .map<Row>((destination) => {
...
export function DestinationSummaryTable({
  ...
  const rows = useMemo(
    () => buildRows(destinationFilter, limit),
    [destinationFilter, limit],
  );
